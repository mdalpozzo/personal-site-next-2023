...
export async function POST(request: NextRequest, res: Response) {
    ...
    if (request.headers.get('content-type') === 'application/json') {
        const { name, email, subject, body } = await request.json() as EmailRequestBody;

        try {
            await sendEmail({ subject: `${name} - ${subject}`, from: email, text: body, to: process.env.GMAIL_EMAIL_ADDRESS });
        ...
        console.error('Send email POST error: content-type is not application/json');
        // res.status(500).json({ error: "Failed to send the email: content-type must be application/json" });
        return new Response('Failed to send the email: content-type must be application/json.', {
            status: 500,
        })
    }
}